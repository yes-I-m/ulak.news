<?php
    ...
    if(isset($_GET['process'])){
        ...
        if($_GET['process'] === "emojiGet"){
         
            $emoji_get = $ulak_api_class->get_emoji($_GET['agency'], $_GET['id']);

            if($emoji_get['status']){
                $main_response['status'] = true;
                $main_response['result'] = $emoji_get['result'];

            }else{
                $main_response['desc'] = "Emoji bilgileri alınamadı!";
            }
            
        }

        if($_GET['process'] === "emojiSave"){
         
            $emoji_save = $ulak_api_class->save_emoji($_GET['agency'], $_GET['id'], $_GET['rate']);
            if($emoji_save['status']){
                $main_response['status'] = true;
                $main_response['desc'] = $emoji_save['desc'];
                $main_response['result'] = $emoji_save['result'];
